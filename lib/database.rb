...
module Database
  ...
  def make_save(save_hash = {}, save_array = [])
    ...
    @gameboard.board.board.flatten.each do |piece|
  ...
  def load_save(filename, load_array = nil)
    File.open("save/#{filename}", 'r') do |file|
  ...
  def load_game
    ...
    board.map! do |piece|
      ...
      @gameboard.check_piece(piece['name'], piece['pos'], piece['colour'], piece['moved'])
    end
    board = board.each_slice(8).to_a
    @gameboard.update_gameboard(board, history)
